...
const ContentCard = ({ title, description, link, image, icon, category }: ContentCardProps) => (
  <a href={link} className="block rounded-lg overflow-hidden shadow-lg transition-transform duration-300 hover:scale-[1.02] card-bg">
    {image && (
      <img
        src={image}
        alt={title}
        ...
        onError={(e) => { 
          ...
          (e.target as HTMLImageElement).src = `https://placehold.co/600x400/${encodeURIComponent('#4F46E5')}/${encodeURIComponent('#6B7280')}?text=No+Image`; 
        }}
      />
    )}
    <div className="p-6">
